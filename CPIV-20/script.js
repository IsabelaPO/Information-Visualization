...
function applyFilters() {
  ...
  const platformsSelected = filteredPlatformData.map(
        ...
        update
          .call((update) =>
            update
              .transition(t)
              .attr("d", (d) =>
                lineGenerator(d[1].sort((a, b) => a.year - b.year))
              )
              .style("stroke", (d) => platformColors[d[0]])
          )
          .on("mouseover", function (event, d) {
